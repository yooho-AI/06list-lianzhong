...
const STORAGE_KEY = 'lianzhong-chats'
...
你现在正在和一位朋友闲聊。请完全以${char.name}的性格和说话方式回复。
规则：
- 用第一人称说话，不要用第三人称叙述
  ...
  return (
    <>
      ...
      >
        ...
        {/* 消息区域 */}
        <div className="lz-chat-area" style={{ flex: 1 }}>
          {messages.length === 0 && !streaming && (
            <div className="lz-placeholder">
              <img
                src={char.portrait}
                alt={char.name}
                style={{
          ...
          {messages.map((msg, i) =>
            msg.role === 'user' ? (
              <div key={i} className="lz-bubble-player">
                {msg.content}
              </div>
            ) : (
              <div key={i} className="lz-npc-row">
                <img
                  src={char.portrait}
                  alt={char.name}
                ...
                />
                <div className="lz-npc-bubble">{msg.content}</div>
              </div>
            ),
          )}
          ...
          {streaming && streamingContent && (
            <div className="lz-npc-row">
              <img
                ...
                style={{ borderColor: char.themeColor }}
              />
              <div className="lz-npc-bubble">{streamingContent}</div>
            </div>
          )}
